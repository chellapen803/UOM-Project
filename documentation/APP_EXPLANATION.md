...
Knowledge Graph (Nodes & Links)
    ↓
[Chat Interface] → RAG Retrieval (optionally GEER-enhanced) → [Gemini API] → Response
         ↓
    [GEER Service] (localhost:8000) - Optional graph-embedding enhanced retrieval (uses R-GCN encoder)
```
...
└─────────────────┘
    ↓
┌───────────────────────────────┐
│ GEER Service (R-GCN Encoder) │ (Render.com) - Python ML Service
│ FastAPI                       │
└───────────────────────────────┘
    ↓
[Chat Interface] → RAG Retrieval (with GEER enhancement) → [Gemini API] → Response
```

**Note**: In production, the GEER service (which uses an R-GCN model to generate graph embeddings) runs on Render.com because Vercel serverless functions are not suitable for long-running Python services with heavy ML dependencies. The service maintains persistent connections to Neo4j and keeps the trained model loaded in memory.
...
1. **GEER (Graph-Embedding Enhanced Retrieval)** (if available):
   - Checks if the GEER service is available (hosted on Render.com)
   - Uses R-GCN–based graph embeddings to find similar entities
   - Leverages graph structure for better context matching
   - Falls back to standard retrieval if GEER is unavailable
...
**Result**: Returns top 3 most relevant text chunks from Neo4j (enhanced with GEER’s graph embeddings if available)
